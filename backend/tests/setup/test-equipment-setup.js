...
async function testEquipmentSetup() {
  try {
    ...
    console.log("\n2️⃣ Checking Equipment Management Schema...");
    ...
    if (tablesExist === equipmentTables.length) {
      console.log("   ✅ Equipment Management schema already exists and is complete");
    } else {
      console.log("   📝 Applying Equipment Management Schema...");
      ...
      try {
        ...
        if (error.message.includes('already exists')) {
          console.log("   ✅ Equipment Management schema already exists");
        } else {
          throw error;
        }
      }
    ...
    console.log("\n3️⃣ Checking Equipment Sample Data...");
    ...
    if (parseInt(equipmentCount.rows[0].count) > 0) {
      console.log(`   ✅ Equipment sample data already loaded (${equipmentCount.rows[0].count} items)`);
    } else {
      console.log("   📝 Loading Equipment Sample Data...");
    ...
    // Reuse the equipment tables array from above
    for (const table of equipmentTables) {
      const result = await query(
        `SELECT EXISTS (
          SELECT FROM information_schema.tables 
