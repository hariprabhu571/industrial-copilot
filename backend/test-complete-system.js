...
try {
  ...
  if (embeddingsWithProviders[0].provider && embeddingsWithProviders[0].embedding) {
    console.log("   ✅ Provider tracking fixed - embedChunks returns provider info");
    console.log(`   ✅ Chunk 0: provider=${embeddingsWithProviders[0].provider}`);
    console.log(`   ✅ Chunk 1: provider=${embeddingsWithProviders[1].provider}`);
  } else {
    console.log("   ❌ Provider tracking not working");
  }

  // Test smart query embedding
